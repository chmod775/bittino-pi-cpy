...
#include "py/mphal.h"       // mp_printf, MP_PYTHON_PRINTER
#include "py/runtime.h"     // mp_obj_t etc.
#include <string.h>
...
// ─────────────────────────────────────────────────────────────────────────────
//  Opcode name table  (extend as you add opcodes)
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
//  _vm_debug_check  — called from inside the dispatch loop, BEFORE execution
//
//  Returns true  → caller should skip execution and return immediately
                     ...
                     uint32_t stack_ptr)
{
    ...
    // ── If already paused for this task, stay paused until step requested ───
    if (vm_debug.paused && vm_debug.paused_task_id == task_id) {
        if (vm_debug.step_requested) {
            // REPL called vm_debug_step() — consume the token and let one op run
            vm_debug.step_requested = false;
            vm_debug.paused         = false;
            // Fall through: execute the opcode, then re-pause below
        } else {
    ...
    if (vm_debug.step_mode)
        pause_reason = "step";
    else if (_is_pc_break(pc))
        pause_reason = "pc_break";
    else if (_is_op_break(opcode))
        pause_reason = "op_break";
    ...
    if (pause_reason) {
        ...
        if (vm_debug.stack_on_pause)
            _dump_stack(stack_ptr);

        mp_printf(MP_PYTHON_PRINTER,
            "[VM DBG] Call vm_debug_step() to advance one opcode.\n\n");

        // Do NOT execute this opcode yet — block until step_requested
        return true;
    }
...
void vm_debug_step(void) {
    ...
    // paused flag will be cleared by _vm_debug_check on next VM tick
    mp_printf(MP_PYTHON_PRINTER,
